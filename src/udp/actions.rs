    ...
    T: serde::Serialize + serde::de::DeserializeOwned + Clone,
{
    if states.contains_key(room_id) {
        if crate::util::epoch_time() - states[room_id].last_time >= 4000 {
            ...
            return crate::udp::messages::ROOM_EXPIRED.to_owned();
        } else {
            return crate::udp::messages::ROOM_ACTIVE.to_owned();
        }
    }
    crate::udp::messages::NO_ROOM.to_owned()
}

pub fn delete_players<T>(
    states: &mut HashMap<String, crate::udp::data::GameState<T>>,
    room_id: &str,
) -> String
where
    T: serde::Serialize + serde::de::DeserializeOwned + Clone,
{
    if let Some(state) = states.get_mut(room_id) {
        ...
        state.names = names;
        return crate::udp::messages::DELETE_PLAYERS_HANDLED.to_owned();
    }
    crate::udp::messages::NO_ROOM.to_owned()
}
